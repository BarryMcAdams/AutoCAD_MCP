...
class ExecutionEngine:
    ...
    def _pre_execution_validation(
        self, result: ExecutionResult, context: dict[str, Any]
    ) -> ExecutionResult:
    ...
    def _execute_with_monitoring(
        self, result: ExecutionResult, context: dict[str, Any], timeout: float
    ) -> ExecutionResult:
    ...
    def _post_execution_analysis(self, result: ExecutionResult) -> ExecutionResult:
        ...
        if result.status == ExecutionStatus.COMPLETED:
            self.performance_monitor.record_operation(
                ...
                success=True,
                metadata={"memory_used_mb": result.memory_used, "cpu_time": result.cpu_time},
            )
        else:
            self.performance_monitor.record_operation(
                "code_execution",
                result.execution_time,
                success=False,
                metadata={"error": result.error_message, "status": result.status.value},
            )
