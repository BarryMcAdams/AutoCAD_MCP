...
class DiagnosticResult:
    ...
    object_analysis: dict[str, Any] | None = None
...
class ErrorDiagnostics:
    ...
    def analyze_error(
        ...
        context_analysis = self._analyze_error_context(error, context, code, local_vars)

        # Generate diagnostic result
        diagnostic = self._generate_diagnostic_result(error_info, matching_rules, context_analysis)
    ...
    def get_diagnostic_summary(self, time_range_hours: int = 24) -> dict[str, Any]:
        ...
        recent_diagnostics = [d for d in self.diagnostic_history if d.timestamp >= cutoff_time]
        ...
        common_issues = sorted(self.known_issues.items(), key=lambda x: x[1], reverse=True)[:10]
    ...
    def _generate_diagnostic_result(
        self,
...
""",
    ...
    def _calculate_performance_impact(self) -> str:
        ...
        performance_issues = [
            d for d in self.diagnostic_history if d.category == DiagnosticCategory.PERFORMANCE
        ]
