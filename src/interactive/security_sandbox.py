...
class SecuritySandbox:
    ...
    def __init__(self, default_policy: SecurityPolicy | None = None):
        """
    ...
    def execute_with_sandbox(
        self, code: str, context: dict[str, Any], session_id: str
    ) -> tuple[bool, Any, str]:
        """
            ...
            if success and result is not None:
                ...
                if len(result_str) > policy.max_output_length:
                    return (
                        False,
                        None,
                        f"Output too long: {len(result_str)} > {policy.max_output_length}",
                    )
    ...
    def get_session_security_status(self, session_id: str) -> dict[str, Any]:
        ...
        if not policy:
            return {"has_policy": False, "security_level": "unknown", "total_violations": 0}
        ...
        for pattern_name, pattern_info in self.advanced_patterns.items():
            if re.search(pattern_info["pattern"], code, re.IGNORECASE):
                ...
                if not pattern_info.get("allowed_levels", []):
                    ...
                    violations.append(
                        f"Pattern '{pattern_name}' not allowed at {policy.level.value} level"
                    )
    ...
    def _create_sandboxed_context(
        self, base_context: dict[str, Any], policy: SecurityPolicy
    ) -> dict[str, Any]:
