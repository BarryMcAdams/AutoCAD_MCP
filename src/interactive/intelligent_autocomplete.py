...
class CodeSuggestion:
    ...
    autocad_object_type: str | None = None
    autocad_method_category: str | None = None
...
class UsagePattern:
    ...
    def record_usage(self, method_name: str, context: CodeContext, suggestion_type: SuggestionType):
    ...
    def get_sequence_score(self, method_name: str) -> float:
        ...
        recent_methods = [
            m for m, t in self.method_sequences if time.time() - t < 300
        ]  # Last 5 minutes
        ...
        return {
            ...
            "Document": {
                "methods": {
                    ...
                    "Save": {"params": [], "returns": "void", "description": "Saves the document"},
                    "SaveAs": {
...
class IntelligentAutoComplete:
    ...
    def get_completions(
        self, context: CodeContext, trigger_character: str | None = None
    ) -> list[CodeSuggestion]:
        """
    ...
    def record_completion_usage(self, suggestion: CodeSuggestion, context: CodeContext):
        """Record usage of a completion suggestion for learning."""
        try:
            self.usage_patterns.record_usage(suggestion.text, context, suggestion.type)
    ...
    def get_signature_help(
        self, context: CodeContext, function_name: str
    ) -> dict[str, Any] | None:
        """
    ...
    def get_hover_information(self, context: CodeContext, symbol: str) -> dict[str, Any] | None:
        """
    ...
    def _analyze_context_type(
        self, context: CodeContext, trigger_character: str | None
    ) -> ContextType:
    ...
    def _get_import_suggestions(self, context: CodeContext) -> list[CodeSuggestion]:
        ...
        autocad_imports = ["win32com.client", "pyautocad", "comtypes.client", "pythoncom"]
    ...
    def _apply_ml_ranking(
        self, suggestions: list[CodeSuggestion], context: CodeContext
    ) -> list[CodeSuggestion]:
    ...
    def _get_cache_key(self, context: CodeContext, trigger_character: str | None) -> str:
        """Generate a cache key for the context."""
        return (
            f"{context.file_path}:{context.line_number}:{context.column_number}:{trigger_character}"
        )
