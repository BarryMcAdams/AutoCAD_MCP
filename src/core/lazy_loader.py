...
class LazyLoader:
    ...
    def __init__(
        self,
        module_name: str,
        fallback_factory: Callable | None = None,
    ...
    def _load_module(self):
        ...
        try:
            ...
            logger.debug(
                f"Successfully loaded optional dependency: {self.module_name}"
            )
    ...
    def get_error(self) -> ImportError | None:
...
def check_dependencies() -> dict[str, dict[str, bool | str]]:
    """
...
if __name__ == "__main__":
    ...
    logger.info("Testing lazy loading framework...")
    ...
    for name, info in dep_status.items():
        ...
        logger.info(
            f"{status_icon} {name}: {'Available' if info['available'] else 'Not available'}"
        )
        if info["error"]:
