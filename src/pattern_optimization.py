...
class Pattern:
    ...
    material_grain_direction: str | None = None  # 'horizontal', 'vertical', or None
...
class MaterialSheet:
    ...
    grain_direction: str | None = None  # 'horizontal', 'vertical', or None
...
class PatternNestingOptimizer:
    ...
    def __init__(self, material_sheets: list[MaterialSheet]):
        ...
        logger.info(
            f"Pattern nesting optimizer initialized with {len(material_sheets)} material types"
        )

    def optimize_nesting(
        self,
        patterns: list[Pattern],
        algorithm: str = "best_fit_decreasing",
        max_sheets: int = 10,
        ...
        try:
            ...
            if algorithm not in self.algorithms:
                raise ValueError(
                    f"Unknown algorithm: {algorithm}. Available: {list(self.algorithms.keys())}"
                )
            ...
            nesting_result = self.algorithms[algorithm](
                sorted_patterns, max_sheets, rotation_angles
            )
            ...
            logger.info(
                f"Nesting optimization completed: {metrics['total_material_utilization']:.1f}% utilization"
            )
    ...
    def _bottom_left_fill(
        self, patterns: list[Pattern], max_sheets: int, rotation_angles: list[float]
    ) -> dict[str, Any]:
        ...
        for sheet_idx in range(max_sheets):
            ...
            for pattern in unplaced_patterns.copy():
                best_placement = self._find_bottom_left_position(
                    pattern, placed_patterns, best_sheet, rotation_angles
                )
                ...
                sheet_layouts.append(
                    {
                        ...
                        "utilization": self._calculate_sheet_utilization(
                            placed_patterns, best_sheet
                        ),
                    }
                )
    ...
    def _best_fit_decreasing(
        ...
        for sheet_idx in range(max_sheets):
            ...
            for pattern in unplaced_patterns.copy():
                best_fit = self._find_best_fit_position(
                    pattern, placed_patterns, best_sheet, rotation_angles
                )
    ...
    def _genetic_algorithm_nest(
        ...
        for generation in range(10):  # Limited generations for performance
            ...
            if total_utilization > best_utilization:
                ...
                best_result = result
                best_result["algorithm_details"] = (
                    f"Genetic algorithm (generation {generation + 1})"
                )

        return best_result or {"sheet_layouts": [], "unplaced_patterns": patterns}

    def _simulated_annealing_nest(
        self, patterns: list[Pattern], max_sheets: int, rotation_angles: list[float]
    ) -> dict[str, Any]:
        ...
        for iteration in range(50):  # Limited iterations for performance
            ...
            neighbor_result = self._generate_neighbor_solution(
                current_result, patterns, max_sheets, rotation_angles
            )
        ...
        for angle in rotation_angles:
            ...
            for y in np.arange(0, sheet.height - height + 1, 5):  # 5mm grid
                for x in np.arange(0, sheet.width - width + 1, 5):
                    ...
                    if self._fits_on_sheet(candidate, sheet) and not self._overlaps_with_placed(
                        candidate, placed_patterns
                    ):
    ...
    def _overlaps_with_placed(
        self, candidate: PlacedPattern, placed_patterns: list[PlacedPattern]
    ) -> bool:
        ...
        for placed in placed_patterns:
            ...
            if not (
                cand_max_x + margin <= placed_min_x
                or placed_max_x + margin <= cand_min_x
                or cand_max_y + margin <= placed_min_y
                or placed_max_y + margin <= cand_min_y
            ):
                return True
    ...
    def _calculate_fit_score(
        self, candidate: PlacedPattern, placed_patterns: list[PlacedPattern], sheet: MaterialSheet
    ) -> float:
    ...
    def _calculate_sheet_utilization(
        self, placed_patterns: list[PlacedPattern], sheet: MaterialSheet
    ) -> float:
    ...
    def _calculate_optimization_metrics(self, nesting_result: dict[str, Any]) -> dict[str, Any]:
        ...
        return {
            ...
            "waste_percentage": (
                ((total_sheet_area - total_pattern_area) / total_sheet_area) * 100.0
...
def create_patterns_from_unfolding_results(
    unfolding_results: list[dict[str, Any]]
) -> list[Pattern]:
    """
    ...
    try:
        for i, result in enumerate(unfolding_results):
            ...
            area = width * height
            if (
                "distortion_metrics" in result
                and "total_surface_area" in result["distortion_metrics"]
            ):
            ...
            logger.info(
                f"Created pattern {pattern.id}: {width:.1f}x{height:.1f}mm, area={area:.1f}mm²"
            )
...
def optimize_material_usage(
    patterns: list[Pattern],
    material_sheets: list[MaterialSheet],
    algorithm: str = "best_fit_decreasing",
) -> dict[str, Any]:
    """
    ...
    try:
        ...
        if result["success"]:
            logger.info(
                f"Material optimization completed: "
