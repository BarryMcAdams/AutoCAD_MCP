...
class MockModelSpace:
    ...
    def AddLine(self, start_point: list[float], end_point: list[float]) -> MockLine:
        ...
        line = MockLine(
            object_id=object_id, start_point=start_point.copy(), end_point=end_point.copy()
        )
        self.entities[object_id] = line

        # Log the operation
        self.parent.log_operation(
            "AddLine", {"start_point": start_point, "end_point": end_point, "object_id": object_id}
        )
    ...
    def AddCircle(self, center: list[float], radius: float) -> MockCircle:
        ...
        object_id = self._get_next_object_id()
        circle = MockCircle(object_id=object_id, center=center.copy(), radius=radius)
        self.entities[object_id] = circle

        # Log the operation
        self.parent.log_operation(
            "AddCircle", {"center": center, "radius": radius, "object_id": object_id}
        )
    ...
    def AddPolyline(self, points: list[list[float]]) -> MockEntity:
        ...
        object_id = self._get_next_object_id()
        polyline = MockEntity(object_id=object_id, entity_type="AcDbPolyline")
        polyline.properties["Vertices"] = [p.copy() for p in points]
        self.entities[object_id] = polyline

        # Log the operation
        self.parent.log_operation("AddPolyline", {"points": points, "object_id": object_id})

        return polyline

    def AddExtrudedSolid(
        self, profile: MockEntity, height: float, taper_angle: float = 0.0
    ) -> MockSolid:
        ...
        solid = MockSolid(object_id=object_id, volume=volume)
    ...
    def AddRevolvedSolid(
        self, profile: MockEntity, axis_point: list[float], axis_vector: list[float], angle: float
    ) -> MockSolid:
    ...
    def Union(self, entities: list[MockEntity]) -> MockSolid:
        ...
        solid = MockSolid(object_id=object_id, volume=total_volume)
    ...
    def Subtract(self, base_entity: MockEntity, subtract_entities: list[MockEntity]) -> MockSolid:
        ...
        solid = MockSolid(object_id=object_id, volume=result_volume)
    ...
    def GetEntity(self, object_id: int) -> MockEntity | None:
...
class MockAutoCAD:
    ...
    def get_call_log(self) -> list[str]:
        ...
        return [f"{entry['operation']}({entry['parameters']})" for entry in self.call_log]
    ...
    def get_entities_by_type(self, entity_type: str) -> list[MockEntity]:
        ...
        return [
            entity for entity in self.model.entities.values() if entity.entity_type == entity_type
        ]
...
def create_test_scenario(scenario_name: str) -> MockAutoCAD:
    ...
    if scenario_name == "empty_drawing":
        ...
        profile = mock_acad.model.AddPolyline(
            [[100, 0, 0], [150, 0, 0], [150, 50, 0], [100, 50, 0]]
        )
