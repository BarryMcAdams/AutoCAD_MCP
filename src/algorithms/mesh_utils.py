...
def extract_triangle_mesh(acad_entity) -> tuple[np.ndarray, np.ndarray]:
    ...
    try:
        logger.info(
            f"Extracting triangle mesh from AutoCAD entity {acad_entity.ObjectID}"
        )
    ...
    try:
        ...
        for i in range(n_vertices):
            if vertex_areas[i] > 1e-12:
                vertex_normals[i] = vertex_normals[i] / np.linalg.norm(
                    vertex_normals[i]
                )
        ...
        logger.info(
            f"Curvature analysis completed: {surface_type} surface, "
...
def validate_mesh_manifold(
    ...
    try:
        ...
        # Overall assessment
        is_suitable = (
            validation_result["is_manifold"]
            and validation_result["n_degenerate_triangles"] == 0
            and validation_result["n_vertices"]
            == validation_result["n_unique_vertices"]
        )
...
def optimize_mesh_for_lscm(
    vertices: np.ndarray, triangles: np.ndarray, max_vertices: int = 10000
    ...
    logger.info(
        f"Mesh too large ({len(vertices)} vertices), applying simplification..."
    )
    ...
    logger.info(
        "Mesh optimized: %s vertices, %s triangles",
        len(optimized_vertices),
        len(optimized_triangles),
    )
