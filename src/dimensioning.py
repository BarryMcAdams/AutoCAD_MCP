...
class DimensioningSystem:
    ...
    def create_linear_dimension(
        ...
        try:
            logger.info(
                f"Creating linear dimension from {start_point} to {end_point}"
            )
            ...
            self.doc.ActiveLayer = self.doc.Layers.Item(
                self.dimension_settings["dimension_layer"]
            )
            ...
            dim_pt = win32com.client.VARIANT(
                win32com.client.pythoncom.VT_ARRAY | win32com.client.pythoncom.VT_R8,
            ...
            dimension.ExtensionLineExtend = self.dimension_settings[
                "extension_line_extend"
            ]
            dimension.ExtensionLineOffset = self.dimension_settings[
            ...
            dimension_info = {
                ...
                "measured_distance": round(
                    distance, self.dimension_settings["precision"]
                ),
            ...
            logger.info(
                "Linear dimension created: %.2f %s",
                distance,
                self.dimension_settings["units"],
            )
    ...
    def create_angular_dimension(
        ...
        try:
            ...
            # Convert points to AutoCAD format
            vertex_pt = win32com.client.VARIANT(
                win32com.client.pythoncom.VT_ARRAY | win32com.client.pythoncom.VT_R8,
                vertex_point,
            )
            first_pt = win32com.client.VARIANT(
                win32com.client.pythoncom.VT_ARRAY | win32com.client.pythoncom.VT_R8,
                first_point,
            )
            second_pt = win32com.client.VARIANT(
                win32com.client.pythoncom.VT_ARRAY | win32com.client.pythoncom.VT_R8,
                second_point,
            )
            ...
            ang_dimension = self.model_space.AddDim3PointAngular(
                vertex_pt, first_pt, second_pt, text_pt
            )
            ...
            # Normalize vectors
            vec1_norm = (
                vec1 / np.linalg.norm(vec1) if np.linalg.norm(vec1) > 1e-12 else vec1
            )
            vec2_norm = (
                vec2 / np.linalg.norm(vec2) if np.linalg.norm(vec2) > 1e-12 else vec2
            )
    ...
    def create_leader_with_text(
        self, start_point: list[float], end_point: list[float], text_content: str
    ) -> dict[str, Any]:
        """
        ...
        try:
            ...
            points_array = win32com.client.VARIANT(
                win32com.client.pythoncom.VT_ARRAY | win32com.client.pythoncom.VT_R8,
                leader_points,
            )

            # Create leader
            leader = self.model_space.AddLeader(
                points_array, None, 1
    ...
    def dimension_unfolded_pattern(
        ...
        try:
            ...
            if "pattern_bounds" in pattern_data:
                ...
                width_dim_line = [
                    min_pt[0] + (max_pt[0] - min_pt[0]) / 2,
                    min_pt[1] - 15,
                    0,
                ]

                width_dim = self.create_linear_dimension(
                    width_start, width_end, width_dim_line
                )
                ...
                height_dim_line = [
                    min_pt[0] - 15,
                    min_pt[1] + (max_pt[1] - min_pt[1]) / 2,
                    0,
                ]

                height_dim = self.create_linear_dimension(
                    height_start, height_end, height_dim_line
                )
            ...
            if "fold_lines" in pattern_data:
                for i, fold_line in enumerate(
                    pattern_data["fold_lines"][:5]
                ):  # Limit to 5 fold lines
                    try:
                        ...
                        fold_dim = self.create_linear_dimension(
                            start_coord, end_coord, dim_line_pt, f"{fold_type.upper()}"
                        )
            ...
            for note in manufacturing_notes:
                note_annotation = self.create_text_annotation(
                    note["position"], note["text"], text_height=2.0
                )
            ...
            logger.info(
                "Pattern dimensioning completed: %s dimensions, %s annotations",
                len(dimensions),
                len(annotations),
            )
    ...
    def generate_manufacturing_notes(
        self, pattern_data: dict[str, Any]
    ) -> list[dict[str, Any]]:
        ...
        try:
            ...
            if "manufacturing_data" in pattern_data:
                ...
                if "distortion_acceptable" in mfg_data:
                    ...
                    notes.append(
                        {"position": [10, 5, 0], "text": f"DISTORTION: {status}"}
                    )
            ...
            notes.append(
                {"position": [10, -5, 0], "text": f"UNFOLDING METHOD: {method}"}
            )
...
def create_manufacturing_drawing(
    autocad_instance: Autocad, pattern_data: dict[str, Any]
) -> dict[str, Any]:
    """
    ...
    try:
        ...
        result = {
            ...
            "total_elements": (
                dimensioning_result.get("dimensions_created", 0)
                + dimensioning_result.get("annotations_created", 0)
            ),
        }

        logger.info(
            "Manufacturing drawing created successfully with %s elements",
