...
class UserSession:
    ...
    # Connection info
    websocket_connection: Any | None = None
    ip_address: str | None = None
    user_agent: str | None = None

    # Session state
    active_files: set[str] = field(default_factory=set)
    current_cursor_position: tuple[str, int, int] | None = None
...
class OperationalTransform:
    ...
    def transform_operation(
        self, op1: EditOperation, op2: EditOperation
    ...
    def _transform_insert_delete(
        self, insert_op: EditOperation, delete_op: EditOperation
    ) -> tuple[EditOperation, EditOperation]:
...
class CollaborationServer:
    ...
    async def start_server(self):
        ...
        try:
            ...
            server = await websockets.serve(self._handle_websocket_connection, self.host, self.port)

            logger.info(f"Collaboration server started on ws://{self.host}:{self.port}")

            # Keep server running
            await asyncio.gather(server.wait_closed(), event_processor_task)
    ...
    def create_workspace(
        ...
        workspace = Workspace(
            id=workspace_id, name=name, owner_id=owner_id, settings=settings or {}
        )
    ...
    def join_workspace(
        self, workspace_id: str, user_id: str, role: UserRole | None = None
    ) -> bool:
        """
        ...
        if user_id in workspace.members:
            ...
            logger.info(
                f"User {user_id} already in workspace {workspace_id} with role {existing_role.value}"
            )
            return True
        ...
        session = UserSession(
            user_id=user_id, session_id=session_id, workspace_id=workspace_id, role=assigned_role
        )
        ...
        logger.info(
            f"User {user_id} joined workspace {workspace_id} with role {assigned_role.value}"
        )
        return True
    ...
    def leave_workspace(self, session_id: str) -> bool:
        ...
        if workspace_id in self.workspaces:
            ...
            files_to_unlock = [
                file_path
                for file_path, lock_user in workspace.file_locks.items()
    ...
    def handle_edit_operation(
        self, session_id: str, file_path: str, operation: EditOperation
    ) -> bool:
        """
    ...
    def get_workspace_state(self, workspace_id: str) -> dict[str, Any] | None:
        """
    ...
    def _apply_operational_transform(
        self, workspace_id: str, file_path: str, operation: EditOperation
    ) -> EditOperation:
