...
class DockerManager:
    ...
    def push_image(
        self, image_name: str, image_tag: str, registry_url: str | None = None
    ) -> bool:
        """
    ...
    def generate_deployment_manifest(self, config: DeploymentConfig) -> str:
        ...
        deployment = {
            ...
            "spec": {
                ...
                "selector": {"matchLabels": {"app": config.name, "version": config.version}},
                "template": {
                    "metadata": {
                        "labels": {
                            "app": config.name,
    ...
    def generate_service_manifest(self, config: DeploymentConfig) -> str:
        ...
        service = {
            ...
            "spec": {
                ...
                "selector": {"app": config.name, "version": config.version},
            },
        }
...
class DeploymentAutomationEngine:
    ...
    def create_deployment_package(
        self, source_dir: str, output_dir: str, include_tests: bool = False
    ) -> str:
        """
    ...
    def deploy_application(self, config: DeploymentConfig) -> DeploymentResult:
        ...
        try:
            ...
            logger.info(
                f"Deployment {deployment_id} completed successfully in {result.deployment_duration:.2f}s"
            )
    ...
    def rollback_deployment(self, deployment_id: str, target_version: str | None = None) -> bool:
        """
    ...
    def _deploy_to_kubernetes(
        self, config: DeploymentConfig, manifests: dict[str, str], result: DeploymentResult
    ) -> bool:
    ...
    def _attempt_rollback(self, config: DeploymentConfig, result: DeploymentResult):
        ...
        try:
            logger.warning("Attempting automatic rollback for failed deployment")
    ...
    def _load_deployment_templates(self) -> dict[str, dict[str, Any]]:
        ...
        return {
            "web_application": {
                ...
                "environment_variables": {"LOG_LEVEL": "INFO", "METRICS_ENABLED": "true"},
            },
            "api_service": {
                "port": 8080,
                "health_check_path": "/api/health",
                "readiness_probe_path": "/api/ready",
                "environment_variables": {"API_VERSION": "v1", "CORS_ENABLED": "true"},
            },
            "background_worker": {
                "port": 8080,
                "health_check_path": "/health",
                "environment_variables": {"WORKER_PROCESSES": "4", "QUEUE_BACKEND": "redis"},
            },
        }
