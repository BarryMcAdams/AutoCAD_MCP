...
End Sub""",
    ...
    def generate_code(
        ...
        if "excel" in description.lower() or target_host == "excel":
            ...
            ui_word in description.lower() for ui_word in ["form", "dialog", "interface"]
        ):
            return self._generate_user_interface(parsed, complexity)
        elif any(
            auto_word in description.lower()
            for auto_word in ["batch", "automate", "process"]
        ):
    ...
    def _parse_description(self, description: str) -> dict[str, Any]:
        ...
        requires_excel = "excel" in desc_lower or any(
            f.endswith((".xlsx", ".xls", ".xlsm")) for f in file_refs
        )
        requires_ui = any(
            word in desc_lower for word in ["form", "dialog", "interface", "user"]
        )
        requires_automation = any(
            word in desc_lower for word in ["batch", "automate", "process"]
        )
    ...
    def _extract_macro_name(self, description: str) -> str:
        ...
        macro_match = re.search(
            r'macro\\s+(?:called\\s+)?["\']?(\\w+)["\']?', description.lower()
        )
        if macro_match:
    ...
    def _generate_excel_integration(
        self, parsed: dict[str, Any], complexity: str
    ) -> dict[str, Any]:
...
End Sub"""
    ...
    def _generate_automation_workflow(
        self, parsed: dict[str, Any], complexity: str
    ) -> dict[str, Any]:
        """Generate automation workflow VBA code."""
        function_name = parsed["macro_name"]

        function_body = f"""    ' Initialize progress tracking
    ...
    {function_name} = (processedItems = totalItems)"""
    ...
    {function_name} = False"""
        ...
        return {
            ...
            "usage_instructions": [
                ...
                "Implement the helper functions: GetTotalItemsToProcess, "
                "ProcessSingleItem, UpdateProgress",
    ...
    def validate_vba_code(self, code: str) -> dict[str, Any]:
        ...
        if "Option Explicit" not in code:
            suggestions.append(
                "Add 'Option Explicit' at the top for better variable handling"
            )
    ...
    def generate_complete_solution(
        self, description: str, complexity: str = "intermediate"
    ) -> dict[str, Any]:
        ...
        return {
            ...
            "dependencies": list(
                {dep for module in modules.values() for dep in module.dependencies}
            ),
