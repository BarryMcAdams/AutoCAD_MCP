...
class ValidationEngine:
    ...
    def validate_code(
        ...
        if language.lower() == "python":
            ...
            return ValidationResult(
                ...
                language=language,
                issues=[
                    ValidationIssue("error", "validation", f"Unsupported language: {language}")
                ],
                quality_score=0,
    ...
    def _check_python_imports(self, code: str) -> List[ValidationIssue]:
        ...
        for pattern in validators["forbidden_patterns"]:
            ...
            for match in matches:
                ...
                if pattern == r"\beval\s*\(" and "ast.literal_eval" in matched_text:
                    # Safe usage with ast.literal_eval - skip this finding
                    logger.info(
                        f"DEBUG: Safe eval() usage detected at line {line_num} - using ast.literal_eval"
                    )
                    continue
                ...
                issues.append(
                    ValidationIssue(
                        severity="error",
                        category="security",
                        message=f"Dangerous pattern detected: {matched_text}",
                        line_number=line_num,
                        suggestion=(
                            "Replace with safer alternative"
                            if pattern != r"\beval\s*\("
                            else "Replace eval() with ast.literal_eval() or remove entirely"
                        ),
                    )
                )
    ...
    def _check_security_issues(self, code: str, language: str) -> List[ValidationIssue]:
        ...
        # DEBUG: Log security pattern matching for analysis
        logger.info(f"DEBUG: Starting security pattern matching for {language}")
        ...
        for pattern in self.common_patterns["security_risks"]:
            ...
            for match in matches:
                line_num = code[: match.start()].count("\n") + 1
                # DEBUG: Log matched patterns
                logger.info(f"DEBUG: Security pattern matched at line {line_num}: {match.group()}")
                issues.append(
                    ValidationIssue(
                        severity="error",
                        category="security",
        ...
        logger.info(f"DEBUG: Security check completed for {language}, found {len(issues)} issues")
        return issues
