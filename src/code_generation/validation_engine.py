...
class ValidationEngine:
    ...
    def validate_code(
        ...
        if language.lower() == "python":
            ...
            return ValidationResult(
                ...
                issues=[
                    ValidationIssue(
                        "error", "validation", f"Unsupported language: {language}"
                    )
                ],
                quality_score=0,
            ...
            if "get_connection_status" not in code:
                issues.append(
                    ValidationIssue(
                        ...
                        suggestion="Add connection status check: "
                        "acad.get_connection_status()['connected']",
                    )
                )
            ...
            if not re.search(pattern, code):
                ...
                issues.append(
                    ValidationIssue(
                        ...
                        suggestion=f"Consider adding {pattern_name} "
                        "for proper AutoLISP structure",
                    )
                )
        ...
        if "getpoint" not in used_functions and "command" in used_functions:
            issues.append(
                ValidationIssue(
                    severity="info",
                    category="style",
                    message="Consider using (getpoint) for user input instead of "
                    "hardcoded coordinates",
                    suggestion='Use (getpoint "Specify point: ") for interactive '
                    "point selection",
                )
            )
                ...
                if "Option Explicit" in pattern:
                    ...
                    issues.append(
                        ValidationIssue(
                            ...
                            suggestion="Add 'On Error GoTo ErrorHandler' for proper "
                            "error handling",
                        )
                    )
    ...
    def _check_vba_error_handling(self, code: str) -> list[ValidationIssue]:
        ...
        has_error_goto = any(
            re.search(pattern, code, re.IGNORECASE) for pattern in error_patterns
        )
            ...
            if not has_error_goto:
                issues.append(
                    ValidationIssue(
                        ...
                        suggestion="Add 'On Error GoTo ErrorHandler' and create "
            ...
            if not re.search(r"Exit\s+(Sub|Function)", code, re.IGNORECASE):
                issues.append(
                    ValidationIssue(
                        ...
                        suggestion="Add 'Exit Sub' or 'Exit Function' before "
                        "ErrorHandler label",
                    )
                )
    ...
    def _check_vba_cleanup(self, code: str) -> list[ValidationIssue]:
        ...
        object_declarations = re.finditer(
            r"Set\s+(\w+)\s*=\s*(GetObject|CreateObject)", code, re.IGNORECASE
        )
                ...
                if "Add reference to AutoCAD Type Library" not in code:
                    issues.append(
                        ValidationIssue(
                            ...
                            suggestion="Ensure AutoCAD Type Library reference is added "
                            "in VBA Editor",
                        )
                    )
            ...
            for match in matches:
                ...
                issues.append(
                    ValidationIssue(
                        severity="error",
                        category="security",
                        message="Potential security risk: hardcoded sensitive data",
                        line_number=line_num,
    ...
    def _create_summary(self, issues: list[ValidationIssue]) -> dict[str, Any]:
        ...
        for issue in issues:
            summary["categories"][issue.category] = (
                summary["categories"].get(issue.category, 0) + 1
            )
    ...
    def _generate_autolisp_suggestions(
        self, issues: list[ValidationIssue], code: str
    ) -> list[str]:
        ...
        if "getpoint" not in code and "command" in code:
            suggestions.append(
                "Consider using interactive input functions like (getpoint)"
            )

        if not re.search(r"\(princ\)", code):
            suggestions.append(
                "End command functions with (princ) to suppress return values"
            )

        return suggestions

    def _generate_vba_suggestions(
        self, issues: list[ValidationIssue], code: str
    ) -> list[str]:
        ...
        if "Option Explicit" not in code:
            suggestions.append(
                "Add 'Option Explicit' to catch variable declaration errors"
            )

        if "On Error GoTo" not in code:
            suggestions.append(
                "Implement proper error handling with 'On Error GoTo ErrorHandler'"
            )

        if "CreateObject" in code or "GetObject" in code:
            suggestions.append(
                "Ensure proper object cleanup with 'Set object = Nothing'"
            )
    ...
    def _detect_language(self, code: str) -> str:
        ...
        if re.search(r"def\s+\w+\s*\(", code) and "import" in code:
            ...
            re.search(r"Sub\s+\w+\s*\(", code, re.IGNORECASE)
            or "Option Explicit" in code
        ):
            return "vba"
        else:
            return "unknown"
    ...
    def generate_validation_report(
        ...
        avg_quality = (
            sum(result.quality_score for result in results.values()) / total_files
            if total_files > 0
            else 0
        )
