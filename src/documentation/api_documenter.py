...
class APIDocumenter:
    ...
    def _analyze_module(
        ...
        try:
            ...
            module = APIModule(name=module_name, description=module_docstring)
                ...
                if isinstance(node, ast.FunctionDef) and not node.name.startswith("_"):
                    ...
                    for item in node.body:
                        if isinstance(item, ast.FunctionDef) and not item.name.startswith(
                            ...
                            method_element = self._extract_function_element(
                                item, str(file_path)
                            )
    ...
    def _extract_function_element(
        self, node: ast.FunctionDef, source_file: str
    ) -> APIElement:
    ...
    def _extract_class_element(
        self, node: ast.ClassDef, source_file: str
    ) -> APIElement:
    ...
    def _extract_import_string(self, node) -> str | None:
        ...
        try:
            ...
            logger.warning(f"Could not extract import string: {e}")
        return None
            ...
            for element in module.elements:
                ...
                for autocad_func in self.autocad_api_map:
                    if autocad_func.lower() in element.name.lower():
                        refs.update(
                            self.autocad_api_map[autocad_func].get("related", [])
                        )
    ...
    def generate_comprehensive_documentation(
        self, output_dir: str, formats: list[str] = None
    ) -> dict[str, str]:
...
<body>
        ...
        for module in self.modules.values():
            for element in module.elements:
                ...
                if any(
                    ...
                    for keyword in ["draw", "add", "create", "line", "circle"]
                ):
                    category = "Drawing"
                elif any(
                    keyword in name_lower
                    for keyword in ["extrude", "revolve", "union", "subtract", "3d"]
                ):
                    category = "3D Modeling"
                elif any(
                    keyword in name_lower for keyword in ["test", "mock", "assert"]
                ):
                    category = "Testing"
                elif any(
                    keyword in name_lower
                    for keyword in ["project", "template", "scaffold"]
                ):
                    category = "Project Management"
                elif any(
                    keyword in name_lower for keyword in ["util", "helper", "manager"]
                ):
                    category = "Utilities"
    ...
    def _generate_nav_links(self, categories: dict[str, list[APIElement]]) -> str:
        ...
        for category in categories.keys():
            links.append(
                f'<li><a href="#category-{category.lower().replace(" ", "-")}">{category}</a></li>'
            )
        return "\\n".join(links)

    def _generate_html_categories(
        self, categories: dict[str, list[APIElement]]
    ) -> str:
        ...
        for category, elements in categories.items():
            ...
            for element in elements:
                ...
                html_parts.append(
                    f'<div class="signature"><code>{element.signature}</code></div>'
                )
                ...
                if element.parameters:
                    ...
                    for param in element.parameters:
                        ...
                        html_parts.append(
                            f'<div class="parameter"><code>{param["name"]}</code>'
                            f'{param_type}: {param["description"]}</div>'
                        )
                    html_parts.append("</div>")

                if element.returns:
                    html_parts.append(
                        f"<p><strong>Returns:</strong> {element.returns}</p>"
                    )
                ...
                if element.examples:
                    ...
                    for example in element.examples:
                        html_parts.append(
                            f'<div class="example"><pre><code>{example}</code></pre></div>'
                        )
                ...
                if full_name in self.cross_references:
                    ...
                    if refs:
                        html_parts.append(
                            '<div class="cross-refs"><strong>See also:</strong> '
                        )
                        ref_links = [
                            f'<a href="#{ref.replace(".", "-")}">{ref}</a>'
                            for ref in refs
                        ]
            ...
            for category, elements in categories.items():
                ...
                for element in elements:
                    ...
                    if element.parameters:
                        ...
                        for param in element.parameters:
                            param_type = (
                                f" ({param['type']})" if param.get("type") else ""
                            )
                            f.write(
                                f"- `{param['name']}`{param_type}: {param['description']}\\n"
                            )
                        f.write("\\n")
    ...
    def _generate_json_documentation(self, output_path: Path) -> str:
        ...
        api_data = {
            "metadata": {
                ...
                "total_elements": sum(
                    len(module.elements) for module in self.modules.values()
                ),
            },
            "modules": {},
    ...
    def _generate_index_file(
        self, output_path: Path, generated_files: dict[str, str]
    ):
...
<body>
    ...
    def generate_api_diff(
        self, old_api_data: dict[str, Any], new_api_data: dict[str, Any]
    ) -> dict[str, Any]:
