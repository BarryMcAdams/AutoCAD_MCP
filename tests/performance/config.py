...
def validate_performance_requirements(
    operation: str, triangle_count: int, execution_time: float, memory_usage_mb: float
) -> dict[str, bool]:
    """
    ...
    if operation == "lscm":
        ...
        if triangle_count < 100:
            results["execution_time"] = (
                execution_time < DEFAULT_THRESHOLDS.lscm_small_mesh_time_limit
            )
        elif triangle_count <= 500:
            results["execution_time"] = (
                execution_time < DEFAULT_THRESHOLDS.lscm_medium_mesh_time_limit
            )
        else:
        ...
        if "basic" in operation:
            results["execution_time"] = (
                execution_time < DEFAULT_THRESHOLDS.basic_tool_response_limit
            )
        else:
            results["execution_time"] = (
                execution_time < DEFAULT_THRESHOLDS.advanced_tool_response_limit
            )
...
if __name__ == "__main__":
    ...
    for case in LSCM_TEST_CASES:
        print(
            f"  {case['name']}: {case['triangle_count']} triangles, "
