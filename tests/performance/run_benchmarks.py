...
from config import (
    DEFAULT_THRESHOLDS, DEFAULT_CONFIG, LSCM_TEST_CASES,
    validate_performance_requirements, generate_performance_report_template
)
...
class BenchmarkRunner:
    ...
    def _run_quick_lscm_tests(self) -> Dict[str, Any]:
        ...
        for case in quick_cases:
            ...
            if metrics.success:
                logger.info(f"  ✓ {metrics.execution_time:.3f}s, {metrics.memory_increase_mb:.1f}MB")
            else:
    ...
    def _run_full_lscm_tests(self) -> Dict[str, Any]:
        ...
        for case in LSCM_TEST_CASES:
            ...
            logger.info(f"  ✓ {avg_time:.3f}s avg, {avg_memory:.1f}MB avg ({len(metrics_samples)} runs)")
    ...
    def _run_basic_server_tests(self) -> Dict[str, Any]:
        ...
        for op_name, op_func in basic_operations:
            ...
            results[op_name] = {
                ...
                "memory_usage_mb": metrics.memory_increase_mb,
                "success": metrics.success,
                "meets_requirements": metrics.execution_time < self.thresholds.basic_tool_response_limit
            }
        ...
        for test_name, current_time in current_metrics.items():
            if test_name in baselines:
                ...
                if regression_ratio > self.config.regression_threshold:
                    logger.warning(f"Performance regression in {test_name}: "
            ...
            if isinstance(tests, dict):
                for test_name, test_result in tests.items():
                    ...
                    if isinstance(test_result, dict):
                        if test_result.get("success", False) and test_result.get("meets_requirements", True):
                            passed_tests += 1
                        else:
                            failed_tests += 1
    ...
    def _generate_recommendations(self, requirements_met: Dict[str, bool]) -> List[str]:
        ...
        if all(requirements_met.values()):
            recommendations.append("Performance requirements met - consider stress testing with larger datasets")
...
def main():
    ...
    parser.add_argument("--full", action="store_true", default=True,
                       help="Run complete benchmark suite (default)")
    parser.add_argument("--stress", action="store_true",
                       help="Run stress tests with large meshes")
