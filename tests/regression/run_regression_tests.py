...
class RegressionTestRunner:
    ...
    def run_all_tests(self) -> Dict[str, Any]:
        ...
        print("=" * 60)
        print(f"Testing 7 basic MCP tools after advanced LSCM integration")
    ...
    def test_basic_tool_functionality(self) -> Dict[str, Any]:
            ...
            try:
                ...
                for tool_name in basic_tools:
                    if tool_name not in mcp.tool_handlers:
                        ...
                        results["details"].append(f"Tool '{tool_name}' not found in handler registry")
                        continue
                    ...
                    try:
                        ...
                        if tool_name == "draw_line":
                            ...
                            result = mcp.tool_handlers[tool_name]([[0, 0], [10, 0], [10, 10], [0, 10]], 5.0)
                            ...
                            result = mcp.tool_handlers[tool_name]([[0, 0], [5, 0], [5, 10]], [0, 0, 0], [0, 0, 10], 360.0)
                        ...
                        if "success" not in response:
                            results["passed"] = False
                            results["details"].append(f"{tool_name}: Missing 'success' field in response")
                            continue
                        
                        if not response.get("success", False):
                            results["passed"] = False
                            results["details"].append(f"{tool_name}: Tool reported failure: {response.get('error', 'Unknown error')}")
                            continue
                        ...
                        if execution_time > max_time:
                            results["passed"] = False
                            results["details"].append(f"{tool_name}: Execution time {execution_time:.3f}s exceeds {max_time}s")
                            continue
        ...
        # Summary
        successful_tools = sum(1 for r in results["tool_results"].values() if r.get("success", False))
    ...
    def test_server_stability(self) -> Dict[str, Any]:
        ...
        try:
            ...
            basic_tool_count = len([t for t in available_tools if t in [
                "draw_line", "draw_circle", "extrude_profile", 
                "revolve_profile", "list_entities", "get_entity_info", "server_status"
            ]])
            ...
            if basic_tool_count < 7:
                results["passed"] = False
                results["details"].append(f"Only {basic_tool_count}/7 basic tools found in registry")
            else:
                results["details"].append(f"All 7 basic tools found in registry")
            
            # Check total tool count (should include advanced tools)
            total_tools = len(available_tools)
            if total_tools < 7:
                results["passed"] = False
                results["details"].append(f"Total tools ({total_tools}) less than minimum basic tools (7)")
            else:
    ...
    def test_backward_compatibility(self) -> Dict[str, Any]:
        ...
        with patch('src.utils.get_autocad_instance') as mock_get_acad:
            ...
            try:
                ...
                required_fields = ["success", "mcp_server", "autocad_connected", "active_document", "message"]
                missing_fields = []
                ...
                for field, expected_type in type_checks:
                    if field in response and not isinstance(response[field], expected_type):
                        type_errors.append(f"{field}: expected {expected_type.__name__}, got {type(response[field]).__name__}")
                ...
                results["summary"] = "Backward compatibility maintained" if results["passed"] else "Compatibility issues found"
        ...
        with patch('src.utils.get_autocad_instance') as mock_get_acad:
            ...
            try:
                ...
                if len(concurrent_results) != 4:
                    results["passed"] = False
                    results["details"].append(f"Expected 4 concurrent results, got {len(concurrent_results)}")
                else:
                ...
                if successful_ops != 4:
                    results["passed"] = False
                    results["details"].append(f"Only {successful_ops}/4 concurrent operations succeeded")
                else:
                ...
                if access_count["count"] != 4:
                    results["passed"] = False
                    results["details"].append(f"Thread safety issue: {access_count['count']} accesses instead of 4")
                else:
                    results["details"].append("Thread safety verified")
                
                results["summary"] = "Concurrent execution working" if results["passed"] else "Concurrency issues detected"
            ...
            try:
                ...
                for tool_name, threshold in performance_tests:
                    ...
                    if avg_time > threshold:
                        results["passed"] = False
                        results["details"].append(f"{tool_name}: avg {avg_time:.4f}s exceeds {threshold:.4f}s")
                    else:
                        results["details"].append(f"{tool_name}: ✅ avg {avg_time:.4f}s (limit {threshold:.4f}s)")
                
                results["summary"] = "Performance within limits" if results["passed"] else "Performance regression detected"
    ...
    def test_memory_stability(self) -> Dict[str, Any]:
        ...
        if memory_increase > error_threshold:
            results["passed"] = False
            results["details"].append(f"❌ Memory increase ({memory_increase:.1f} MB) exceeds error threshold ({error_threshold} MB)")
        elif memory_increase > warning_threshold:
            results["details"].append(f"⚠️  Memory increase ({memory_increase:.1f} MB) exceeds warning threshold ({warning_threshold} MB)")
        else:
            results["details"].append(f"✅ Memory increase within acceptable limits")
        ...
        with patch('src.utils.get_autocad_instance') as mock_get_acad:
            ...
            try:
                ...
                if test_memory_increase > leak_threshold:
                    results["passed"] = False
                    results["details"].append(f"❌ Potential memory leak: {test_memory_increase:.1f} MB increase during test")
                else:
                    results["details"].append(f"✅ No memory leak detected: {test_memory_increase:.1f} MB increase")
                
            except Exception as e:
                results["details"].append(f"Memory leak test failed: {e}")
        
        results["summary"] = f"Memory stable (+{memory_increase:.1f} MB)" if results["passed"] else f"Memory issues detected (+{memory_increase:.1f} MB)"
    ...
    def generate_summary(self):
        ...
        print(f"Memory usage: {self.initial_memory:.1f} → {self.results['summary']['final_memory']:.1f} MB "
...
def main():
    ...
    parser.add_argument("--memory-check", "-m", action="store_true", help="Run only memory stability tests")
